...
if(filter_input(INPUT_POST, 'add_to_cart')){
    if(isset($_SESSION['shopping_cart'])){
        ...
        $specialoffers_ids = array_column($_SESSION['shopping_cart'], 'id');
        
        if (!in_array(filter_input(INPUT_GET, 'id'), $specialoffers_ids)){
        $_SESSION['shopping_cart'][$count] = array
            ...
            for ($i = 0; $i < count($specialoffers_ids); $i++){
                if ($specialoffers_ids[$i] == filter_input(INPUT_GET, 'id')){
...
if(filter_input(INPUT_GET, 'action') == 'delete'){
    ...
    foreach($_SESSION['shopping_cart'] as $key => $specialoffers){
        if ($specialoffers['id'] == filter_input(INPUT_GET, 'id')){
            //remove product from the shopping cart when it matches with the GET id
            unset($_SESSION['shopping_cart'][$key]);
        }
    }
    //reset session array keys so they match with $specialoffers_ids numeric array
...
<html>
    <head>
        <title>Special Offers - Robo Express</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
        <!-- CSS -->
        <link rel="stylesheet" type="text/css" href="SpecialOffers_Style.css">
        <!-- jquery -->
...
<!--                                <a class="dropdown-item">Starter Kits</a>-->
                                <a class="dropdown-item" href="#">Development Boards</a>
                                <a class="dropdown-item" href="Arduino.php">Arduino</a>
                                <a class="dropdown-item" href="#">FPGA</a>
                                <a class="dropdown-item" href="#">Robotics</a>
                                <a class="dropdown-item" href="#">GSM/GPS/GPRS</a>
                                <a class="dropdown-item" href="#">Modules</a>
                                <a class="dropdown-item" href="#">Micro Controllers</a>
...
<!--            <div class="hero-text">-->
                <h1 class="specialoffers_products" style="color: aqua">special<span> OFFERS </span></h1>
<!--            </div>-->
        </div>
        ...
        $query = 'SELECT * FROM specialoffers ORDER by id ASC';
        $result = mysqli_query($connect, $query);

        if ($result):
            if(mysqli_num_rows($result)>0):
                while($specialoffers = mysqli_fetch_assoc($result)):
                //prfloat_r($specialoffers);
                ?>
                <div class="col-sm-4 col-md-3" >
                    <form method="post" action="SpecialOffers.php?action=add&id=<?php echo $specialoffers['id']; ?>">
                        <div class="products">
                            <img src="<?php echo $specialoffers['image']; ?>" class="img-responsive" />
                            <h4 class="text-info"><?php echo $specialoffers['title']; ?></h4>
                            <h6><?php echo $specialoffers['modelno'];?></h6>
                            <h5 style="color:green">BDT <?php echo $specialoffers['previousprice']; ?></h5>
                            <h5 style="color:red">BDT <?php echo $specialoffers['afterprice']; ?> (20% OFF!)</h5>
                            <input type="text" name="quantity" class="form-control" value="1" />
                            <input type="hidden" name="title" value="<?php echo $specialoffers['title']; ?>" />
                            <input type="hidden" name="price" value="<?php echo $specialoffers['price']; ?>" />
        ...
        ?>  
        <tr>  
           <td><?php echo $specialoffers['title']; ?></td>  
           <td><?php echo $specialoffers['quantity']; ?></td>  
           <td>BDT <?php echo $specialoffers['price']; ?></td>  
           <td>BDT <?php echo number_format((float)$specialoffers['quantity']* (float)$specialoffers['price'], 2); ?></td>  
           <td>
               <a href="SpecialOffers.php?action=delete&id=<?php echo $specialoffers['id']; ?>">
                    <div class="btn-danger">Remove</div>
               </a>
           </td>  
        </tr>  
        <?php  
                  (float)$total = $total + ((float)$specialoffers['quantity']* (float)$specialoffers['price']);  
             endforeach;  
        ?>  
        <tr>  

...
if(filter_input(INPUT_POST, 'add_to_cart')){
    if(isset($_SESSION['shopping_cart'])){
        ...
        $arduino_ids = array_column($_SESSION['shopping_cart'], 'id');
        
        if (!in_array(filter_input(INPUT_GET, 'id'), $arduino_ids)){
        $_SESSION['shopping_cart'][$count] = array
            ...
            for ($i = 0; $i < count($arduino_ids); $i++){
                if ($arduino_ids[$i] == filter_input(INPUT_GET, 'id')){
...
if(filter_input(INPUT_GET, 'action') == 'delete'){
    ...
    foreach($_SESSION['shopping_cart'] as $key => $arduino){
        if ($arduino['id'] == filter_input(INPUT_GET, 'id')){
            //remove product from the shopping cart when it matches with the GET id
            unset($_SESSION['shopping_cart'][$key]);
        }
    }
    //reset session array keys so they match with $arduino_ids numeric array
...
<html>
    <head>
        <title>Arduino - Robo Express</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
        <!-- CSS -->
        <link rel="stylesheet" type="text/css" href="Arduino_Style.css">
        <!-- jquery -->
...
<!--            <div class="hero-text">-->
                <h1 class="arduino_products" style="color: aqua">categories<span> -  ARDUINO </span></h1>
<!--            </div>-->
        </div>
        ...
        $query = 'SELECT * FROM arduino ORDER by id ASC';
        $result = mysqli_query($connect, $query);

        if ($result):
            if(mysqli_num_rows($result)>0):
                while($arduino = mysqli_fetch_assoc($result)):
                //prfloat_r($arduino);
                ?>
                <div class="col-sm-4 col-md-3" >
                    <form method="post" action="Arduino.php?action=add&id=<?php echo $arduino['id']; ?>">
                        <div class="products">
                            <img src="<?php echo $arduino['image']; ?>" class="img-responsive" />
                            <h4 class="text-info"><?php echo $arduino['title']; ?></h4>
                            <h6><?php echo $arduino['modelno'];?></h6>
                            <h5 style="color:red">BDT <?php echo $arduino['price']; ?></h5>
                    
                            <input type="text" name="quantity" class="form-control" value="1" />
                            <input type="hidden" name="title" value="<?php echo $arduino['title']; ?>" />
                            <input type="hidden" name="price" value="<?php echo $arduino['price']; ?>" />
        ...
        ?>  
        <tr>  
           <td><?php echo $arduino['title']; ?></td>  
           <td><?php echo $arduino['quantity']; ?></td>  
           <td>BDT <?php echo $arduino['price']; ?></td>  
           <td>BDT <?php echo number_format((float)$arduino['quantity']* (float)$arduino['price'], 2); ?></td>  
           <td>
               <a href="    Arduino.php?action=delete&id=<?php echo $arduino['id']; ?>">
                    <div class="btn-danger">Remove</div>
               </a>
           </td>  
        </tr>  
        <?php  
                  (float)$total = $total + ((float)$arduino['quantity']* (float)$arduino['price']);  
             endforeach;  
        ?>  
        <tr>  

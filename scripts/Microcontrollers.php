...
if(filter_input(INPUT_POST, 'add_to_cart')){
    if(isset($_SESSION['shopping_cart'])){
        ...
        $microcontrollers_ids = array_column($_SESSION['shopping_cart'], 'id');
        
        if (!in_array(filter_input(INPUT_GET, 'id'), $microcontrollers_ids)){
        $_SESSION['shopping_cart'][$count] = array
            ...
            for ($i = 0; $i < count($microcontrollers_ids); $i++){
                if ($microcontrollers_ids[$i] == filter_input(INPUT_GET, 'id')){
...
if(filter_input(INPUT_GET, 'action') == 'delete'){
    ...
    foreach($_SESSION['shopping_cart'] as $key => $microcontrollers){
        if ($microcontrollers['id'] == filter_input(INPUT_GET, 'id')){
            //remove product from the shopping cart when it matches with the GET id
            unset($_SESSION['shopping_cart'][$key]);
        }
    }
    //reset session array keys so they match with $microcontrollers_ids numeric array
...
<html>
    <head>
        <title>Microcontrollers - Robo Express</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
        <!-- CSS -->
        <link rel="stylesheet" type="text/css" href="Microcontrollers_Style.css">
        <!-- jquery -->
...
<!--            <div class="hero-text">-->
                <h1 class="microcontrollers_products" style="color: aqua">categories<span> -  MICROCONTROLLERS </span></h1>
<!--            </div>-->
        </div>
        ...
        $query = 'SELECT * FROM microcontrollers ORDER by id ASC';
        $result = mysqli_query($connect, $query);

        if ($result):
            if(mysqli_num_rows($result)>0):
                while($microcontrollers = mysqli_fetch_assoc($result)):
                //prfloat_r($microcontrollers);
                ?>
                <div class="col-sm-4 col-md-3" >
                    <form method="post" action="Microcontrollers.php?action=add&id=<?php echo $microcontrollers['id']; ?>">
                        <div class="products">
                            <img src="<?php echo $microcontrollers['image']; ?>" class="img-responsive" />
                            <h4 class="text-info"><?php echo $microcontrollers['title']; ?></h4>
                            <h6><?php echo $microcontrollers['modelno'];?></h6>
                            <h5 style="color:red">BDT <?php echo $microcontrollers['price']; ?></h5>
                    
                            <input type="text" name="quantity" class="form-control" value="1" />
                            <input type="hidden" name="title" value="<?php echo $microcontrollers['title']; ?>" />
                            <input type="hidden" name="price" value="<?php echo $microcontrollers['price']; ?>" />
        ...
        ?>  
        <tr>  
           <td><?php echo $microcontrollers['title']; ?></td>  
           <td><?php echo $microcontrollers['quantity']; ?></td>  
           <td>BDT <?php echo $microcontrollers['price']; ?></td>  
           <td>BDT <?php echo number_format((float)$microcontrollers['quantity']* (float)$microcontrollers['price'], 2); ?></td>  
           <td>
               <a href="Microcontrollers.php?action=delete&id=<?php echo $microcontrollers['id']; ?>">
                    <div class="btn-danger">Remove</div>
               </a>
           </td>  
        </tr>  
        <?php  
                  (float)$total = $total + ((float)$microcontrollers['quantity']* (float)$microcontrollers['price']);  
             endforeach;  
        ?>  
        <tr>  

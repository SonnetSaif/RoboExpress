...
if(filter_input(INPUT_POST, 'add_to_cart')){
    if(isset($_SESSION['shopping_cart'])){
        ...
        $sensors_ids = array_column($_SESSION['shopping_cart'], 'id');
        
        if (!in_array(filter_input(INPUT_GET, 'id'), $sensors_ids)){
        $_SESSION['shopping_cart'][$count] = array
            ...
            for ($i = 0; $i < count($sensors_ids); $i++){
                if ($sensors_ids[$i] == filter_input(INPUT_GET, 'id')){
...
if(filter_input(INPUT_GET, 'action') == 'delete'){
    ...
    foreach($_SESSION['shopping_cart'] as $key => $sensors){
        if ($sensors['id'] == filter_input(INPUT_GET, 'id')){
            //remove product from the shopping cart when it matches with the GET id
            unset($_SESSION['shopping_cart'][$key]);
        }
    }
    //reset session array keys so they match with $sensors_ids numeric array
...
<html>
    <head>
        <title>Sensors - Robo Express</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
        <!-- CSS -->
        <link rel="stylesheet" type="text/css" href="Sensors_Style.css">
        <!-- jquery -->
...
<!--            <div class="hero-text">-->
                <h1 class="sensors_products" style="color: aqua">categories<span> -  SENSORS </span></h1>
<!--            </div>-->
        </div>
        ...
        if ($result):
            if(mysqli_num_rows($result)>0):
                ...
                <div class="col-sm-4 col-md-3" >
                    <form method="post" action="Sensors.php?action=add&id=<?php echo $sensors['id']; ?>">
                        <div class="products">
                            <img src="<?php echo $sensors['image']; ?>" class="img-responsive" />
                            <h4 class="text-info"><?php echo $sensors['title']; ?></h4>
                            <h6><?php echo $sensors['modelno'];?></h6>
                            <h5 style="color:red">BDT <?php echo $sensors['price']; ?></h5>
                    
                            <input type="text" name="quantity" class="form-control" value="1" />
                            <input type="hidden" name="title" value="<?php echo $sensors['title']; ?>" />
                            <input type="hidden" name="price" value="<?php echo $sensors['price']; ?>" />
        ...
        ?>  
        <tr>  
           <td><?php echo $sensors['title']; ?></td>  
           <td><?php echo $sensors['quantity']; ?></td>  
           <td>BDT <?php echo $sensors['price']; ?></td>  
           <td>BDT <?php echo number_format((float)$sensors['quantity']* (float)$sensors['price'], 2); ?></td>  
           <td>
               <a href="    Sensors.php?action=delete&id=<?php echo $sensors['id']; ?>">
                    <div class="btn-danger">Remove</div>
               </a>
           </td>  
        </tr>  
        <?php  
                  (float)$total = $total + ((float)$sensors['quantity']* (float)$sensors['price']);  
             endforeach;  
        ?>  
        <tr>  

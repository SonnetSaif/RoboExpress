...
if(filter_input(INPUT_POST, 'add_to_cart')){
    if(isset($_SESSION['shopping_cart'])){
        ...
        $fpga_ids = array_column($_SESSION['shopping_cart'], 'id');
        
        if (!in_array(filter_input(INPUT_GET, 'id'), $fpga_ids)){
        $_SESSION['shopping_cart'][$count] = array
            ...
            for ($i = 0; $i < count($fpga_ids); $i++){
                if ($fpga_ids[$i] == filter_input(INPUT_GET, 'id')){
...
if(filter_input(INPUT_GET, 'action') == 'delete'){
    ...
    foreach($_SESSION['shopping_cart'] as $key => $fpga){
        if ($fpga['id'] == filter_input(INPUT_GET, 'id')){
            //remove product from the shopping cart when it matches with the GET id
            unset($_SESSION['shopping_cart'][$key]);
        }
    }
    //reset session array keys so they match with $fpga_ids numeric array
...
<html>
    <head>
        <title>FPGA - Robo Express</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
        <!-- CSS -->
        <link rel="stylesheet" type="text/css" href="FPGA_Style.css">
        <!-- jquery -->
...
<!--            <div class="hero-text">-->
                <h1 class="fpga_products" style="color: aqua">categories<span> -  FPGA </span></h1>
<!--            </div>-->
        </div>
        ...
        $query = 'SELECT * FROM fpga ORDER by id ASC';
        $result = mysqli_query($connect, $query);

        if ($result):
            if(mysqli_num_rows($result)>0):
                while($fpga = mysqli_fetch_assoc($result)):
                //prfloat_r($fpga);
                ?>
                <div class="col-sm-4 col-md-3" >
                    <form method="post" action="FPGA.php?action=add&id=<?php echo $fpga['id']; ?>">
                        <div class="products">
                            <img src="<?php echo $fpga['image']; ?>" class="img-responsive" />
                            <h4 class="text-info"><?php echo $fpga['title']; ?></h4>
                            <h6><?php echo $fpga['modelno'];?></h6>
                            <h5 style="color:red">BDT <?php echo $fpga['price']; ?></h5>
                    
                            <input type="text" name="quantity" class="form-control" value="1" />
                            <input type="hidden" name="title" value="<?php echo $fpga['title']; ?>" />
                            <input type="hidden" name="price" value="<?php echo $fpga['price']; ?>" />
        ...
        ?>  
        <tr>  
           <td><?php echo $fpga['title']; ?></td>  
           <td><?php echo $fpga['quantity']; ?></td>  
           <td>BDT <?php echo $fpga['price']; ?></td>  
           <td>BDT <?php echo number_format((float)$fpga['quantity']* (float)$fpga['price'], 2); ?></td>  
           <td>
               <a href="FPGA.php?action=delete&id=<?php echo $fpga['id']; ?>">
                    <div class="btn-danger">Remove</div>
               </a>
           </td>  
        </tr>  
        <?php  
                  (float)$total = $total + ((float)$fpga['quantity']* (float)$fpga['price']);  
             endforeach;  
        ?>  
        <tr>  

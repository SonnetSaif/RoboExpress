...
if(filter_input(INPUT_POST, 'add_to_cart')){
    if(isset($_SESSION['shopping_cart'])){
        ...
        $burnersprogrammers_ids = array_column($_SESSION['shopping_cart'], 'id');
        
        if (!in_array(filter_input(INPUT_GET, 'id'), $burnersprogrammers_ids)){
        $_SESSION['shopping_cart'][$count] = array
            ...
            for ($i = 0; $i < count($burnersprogrammers_ids); $i++){
                if ($burnersprogrammers_ids[$i] == filter_input(INPUT_GET, 'id')){
...
if(filter_input(INPUT_GET, 'action') == 'delete'){
    ...
    foreach($_SESSION['shopping_cart'] as $key => $burnersprogrammers){
        if ($burnersprogrammers['id'] == filter_input(INPUT_GET, 'id')){
            //remove product from the shopping cart when it matches with the GET id
            unset($_SESSION['shopping_cart'][$key]);
        }
    }
    //reset session array keys so they match with $burnersprogrammers_ids numeric array
...
<html>
    <head>
        <title>Burners/Programmers - Robo Express</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
        <!-- CSS -->
        <link rel="stylesheet" type="text/css" href="BurnersProgrammers_Style.css">
        <!-- jquery -->
...
<!--            <div class="hero-text">-->
                <h1 class="burnersprogrammers_products" style="color: aqua">categories<span> -  BURNERS/PROGRAMMERS </span></h1>
<!--            </div>-->
        </div>
        ...
        $query = 'SELECT * FROM burnersprogrammers ORDER by id ASC';
        $result = mysqli_query($connect, $query);

        if ($result):
            if(mysqli_num_rows($result)>0):
                while($burnersprogrammers = mysqli_fetch_assoc($result)):
                //prfloat_r($burnersprogrammers);
                ?>
                <div class="col-sm-4 col-md-3" >
                    <form method="post" action="BurnersProgrammers.php?action=add&id=<?php echo $burnersprogrammers['id']; ?>">
                        <div class="products">
                            <img src="<?php echo $burnersprogrammers['image']; ?>" class="img-responsive" />
                            <h4 class="text-info"><?php echo $burnersprogrammers['title']; ?></h4>
                            <h6><?php echo $burnersprogrammers['modelno'];?></h6>
                            <h5 style="color:red">BDT <?php echo $burnersprogrammers['price']; ?></h5>
                    
                            <input type="text" name="quantity" class="form-control" value="1" />
                            <input type="hidden" name="title" value="<?php echo $burnersprogrammers['title']; ?>" />
                            <input type="hidden" name="price" value="<?php echo $burnersprogrammers['price']; ?>" />
        ...
        ?>  
        <tr>  
           <td><?php echo $burnersprogrammers['title']; ?></td>  
           <td><?php echo $burnersprogrammers['quantity']; ?></td>  
           <td>BDT <?php echo $burnersprogrammers['price']; ?></td>  
           <td>BDT <?php echo number_format((float)$burnersprogrammers['quantity']* (float)$burnersprogrammers['price'], 2); ?></td>  
           <td>
               <a href="BurnersProgrammers.php?action=delete&id=<?php echo $burnersprogrammers['id']; ?>">
                    <div class="btn-danger">Remove</div>
               </a>
           </td>  
        </tr>  
        <?php  
                  (float)$total = $total + ((float)$burnersprogrammers['quantity']* (float)$burnersprogrammers['price']);  
             endforeach;  
        ?>  
        <tr>  

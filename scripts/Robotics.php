...
if(filter_input(INPUT_POST, 'add_to_cart')){
    if(isset($_SESSION['shopping_cart'])){
        ...
        $robotics_ids = array_column($_SESSION['shopping_cart'], 'id');
        
        if (!in_array(filter_input(INPUT_GET, 'id'), $robotics_ids)){
        $_SESSION['shopping_cart'][$count] = array
            ...
            for ($i = 0; $i < count($robotics_ids); $i++){
                if ($robotics_ids[$i] == filter_input(INPUT_GET, 'id')){
...
if(filter_input(INPUT_GET, 'action') == 'delete'){
    ...
    foreach($_SESSION['shopping_cart'] as $key => $robotics){
        if ($robotics['id'] == filter_input(INPUT_GET, 'id')){
            //remove product from the shopping cart when it matches with the GET id
            unset($_SESSION['shopping_cart'][$key]);
        }
    }
    //reset session array keys so they match with $robotics_ids numeric array
...
<html>
    <head>
        <title>Robotics - Robo Express</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
        <!-- CSS -->
        <link rel="stylesheet" type="text/css" href="Robotics_Style.css">
        <!-- jquery -->
...
<!--            <div class="hero-text">-->
                <h1 class="robotics_products" style="color: aqua">categories<span> -  ROBOTICS </span></h1>
<!--            </div>-->
        </div>
        ...
        $query = 'SELECT * FROM robotics ORDER by id ASC';
        $result = mysqli_query($connect, $query);

        if ($result):
            if(mysqli_num_rows($result)>0):
                while($robotics = mysqli_fetch_assoc($result)):
                //prfloat_r($robotics);
                ?>
                <div class="col-sm-4 col-md-3" >
                    <form method="post" action="Robotics.php?action=add&id=<?php echo $robotics['id']; ?>">
                        <div class="products">
                            <img src="<?php echo $robotics['image']; ?>" class="img-responsive" />
                            <h4 class="text-info"><?php echo $robotics['title']; ?></h4>
                            <h6><?php echo $robotics['modelno'];?></h6>
                            <h5 style="color:red">BDT <?php echo $robotics['price']; ?></h5>
                    
                            <input type="text" name="quantity" class="form-control" value="1" />
                            <input type="hidden" name="title" value="<?php echo $robotics['title']; ?>" />
                            <input type="hidden" name="price" value="<?php echo $robotics['price']; ?>" />
        ...
        ?>  
        <tr>  
           <td><?php echo $robotics['title']; ?></td>  
           <td><?php echo $robotics['quantity']; ?></td>  
           <td>BDT <?php echo $robotics['price']; ?></td>  
           <td>BDT <?php echo number_format((float)$robotics['quantity']* (float)$robotics['price'], 2); ?></td>  
           <td>
               <a href="Robotics.php?action=delete&id=<?php echo $robotics['id']; ?>">
                    <div class="btn-danger">Remove</div>
               </a>
           </td>  
        </tr>  
        <?php  
                  (float)$total = $total + ((float)$robotics['quantity']* (float)$robotics['price']);  
             endforeach;  
        ?>  
        <tr>  
